...
export const WeeklyReport: React.FC<WeeklyReportProps> = ({ tickets, employeeStats }) => {
  ...
  const exportReport = () => {
    ...
    const summaryData = [
      ...
      ['SLA Violations', weeklyData.slaViolations || 0],
    ...
    if (weeklyData.employeeDetails.length > 0) {
      const employeeData = [
        ['Employee Performance Details', '', '', '', '', '', '', '', ''],
        ['Employee', 'Total Interactions', 'Unique Tickets', 'Avg Score', 'Positive', 'Negative', 'Neutral', 'Mixed', 'SLA Violations'],
  ...
  // Calculate SLA compliance using the same method as Dashboard
  const slaCompliance = useMemo(() => {
    if (!weeklyData || weeklyData.totalTickets === 0) return 100;
    ...
    return totalInteractions > 0 
      ...
      : 100;
  }, [selectedWeek, tickets, weeklyData]);
          ...
          <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 rounded-xl p-6 border border-purple-500/20 hover:border-purple-400/30 transition-all duration-300 shadow-lg hover:shadow-purple-500/10">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-white mt-1">{slaCompliance.toFixed(1)}%</p>
